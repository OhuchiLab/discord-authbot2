...
| F11 | 学生情報の一覧 | 管理者 | `/list_students` | 登録されている学生情報 (氏名・学籍番号・学年・メールアドレス・認証の状態) を学年順に表示する。学年や認証の状態で絞り込める |
| F12 | 学生情報の削除 | 管理者 | `/delete_student` | 学籍番号かメンバーで指定した学生情報を、内容を確認してから削除する。認証済みの人は未認証の状態に戻す |
| F13 | 学生情報の書き出し | 管理者 | `/export_students` | 学生情報を CSV (Excel で確認用) または msgpack (復元用) のファイルにして、実行した管理者に渡す |
| F14 | 自動バックアップ | (自動) | 学生情報の保存・Bot の起動 | 学生情報ファイルの日時付きのコピーを残す。古いものから決められた数だけ残して消す |
| F15 | 学生情報の一括登録 | 管理者 | `/import_students` | CSV ファイルの学生をまとめて登録する。登録する人を確認してから確定する。1 行でも問題があれば何も登録しない |
| F16 | 変更履歴のログ | (自動) | 管理者の操作・認証完了・Bot の起動 | 「誰が・何を・どう変えたか」をログ用チャンネル (`authbot-logs`) に投稿する |

## 5. ロール

| ロール名 | 付与されるタイミング | 用途 |
| --- | --- | --- |
| `Administrator` | 手動で付与 | `/register`・`/update_grades`・`/edit_student`・`/list_students`・`/delete_student`・`/export_students`・`/import_students` を実行できる |
...
1. 管理者が `/delete_student` を実行し、対象を `student_number` か `member` のどちらか一方で指定する。
2. Bot が削除する学生情報をすべて表示し、「この操作は取り消せません」と注意する。この時点では何も変更しない。
3. 管理者が [削除する] を押すと学生情報を削除する。認証済みの人だった場合は、Discord 上で未認証の状態に戻す (Authorized・学年ロールを外し、Unauthorized を付ける。ニックネームはそのまま)。
   [キャンセル] または 14 分間操作が無ければ何も変更しない。
4. 削除した学籍番号・メールアドレスは、もう一度 `/register` で登録できる。

### 6.6 学生情報の一括登録 (F15)

1. 管理者が CSV ファイルを用意する ([ひな形](./templates/import_students.csv))。
   - 1 行目は見出しで、「氏名」「学籍番号」「学年」「メールアドレス」の列が必要 (順番は自由。ほかの列は無視する。`/export_students` の CSV と同じ形でよい)。
   - 文字コードは UTF-8 か Shift_JIS (日本語版 Excel で「CSV」として保存したもの) のどちらでもよい。
   - 学年は B4 / M1 / M2 / D1 / D2 / D3 / TEACHER / OB/OG (大文字・小文字は問わない)。
2. `/import_students file:(CSV)` を実行すると、Bot が全行を `/register` と同じ規則 (形式・登録済みとの重複) と、CSV の中での重複で確認する。
   - 1 行でも問題があれば、「3 行目: 学籍番号は英数字 8 文字で…」のように問題の一覧を表示し、**何も登録しない**。CSV を直して再実行する。
   - 問題が無ければ、登録する人の一覧を表示する。この時点では何も登録しない。
3. 管理者が [登録する] を押すと、全員をまとめて登録する (1 回の保存で登録するので、バックアップも 1 つだけ作られる)。
   [キャンセル] または 14 分間操作が無ければ何も登録しない。
4. 上限: ファイルは 1MB、学生は 1 回 500 人まで。

### 6.7 変更履歴のログ (F16)

次の出来事を、ログ用チャンネル (既定 `authbot-logs`、`LOG_CHANNEL_NAME` で変更可) に投稿する。キャンセルした操作は記録しない。

| 出来事 | 投稿の例 |
| --- | --- |
| 登録 (`/register`) | 📝 学生情報の登録: @管理者 が 山田 太郎 (AB123456) を登録しました (学年: B4) |
| 一括登録 (`/import_students`) | 📝 学生情報の一括登録: @管理者 が 2 人を登録しました (続けて 1 人 1 行) |
| 変更 (`/edit_student`) | ✏️ 学生情報の変更: @管理者 が 山田 太郎 (AB123456) を変更しました / 学年: B4 → M1 |
| 削除 (`/delete_student`) | 🗑️ 学生情報の削除: @管理者 が 山田 太郎 (AB123456) を削除しました / Discord: @山田 (未認証に戻しました) |
| 年度更新 (`/update_grades`) | 🎓 2027年度の現役更新: @管理者 が 12 人の学年を更新しました (続けて 1 人 1 行。管理者が変えたものに ✏️) |
| 書き出し (`/export_students`) | 📤 学生情報の書き出し: @管理者 が 30 人分を CSV で書き出しました |
| 認証完了 | ✅ 認証: @山田 が 山田 太郎 (AB123456) として認証されました (学年: B4) |
| Bot の起動 | 🟢 Bot を起動しました。 |

- 投稿の中のメンション (@名前) では、本人に通知しない。
- 2000 文字を超えるログは、行の区切りで分けて投稿する。
- 投稿できなくても (チャンネルが無い・権限が無い)、操作は成功させ、Bot のログに警告を残す。
- **ログには氏名・学籍番号が含まれるため、ログ用チャンネルは管理者と Bot だけが見られる設定にする。**
...
- バックアップ: 保存のたびと起動時に、日時付きのコピーを `data/backups/` (変更可) に残す。新しいものから 50 個 (変更可) を残す。
  **既定の保存先は元のファイルと同じディスクのため、ディスクの故障に備えるなら外付けディスクや NAS を `BACKUP_DIR` に指定する。**
...
| ログ用チャンネル | `authbot-logs` を作り、管理者と Bot だけが見られるようにする。Bot に「チャンネルを見る」「メッセージを送信」を許可する |

## 9. 非機能要件

| 分類 | 方針 |
| --- | --- |
| 永続性 | 学生情報は変更のたびにファイルへ保存する。一時ファイルに書いてから置き換えるため、保存中に停止してもファイルは壊れない。保存のたびに日時付きのバックアップを残し、誤操作の前の状態に戻せる (F14) |
