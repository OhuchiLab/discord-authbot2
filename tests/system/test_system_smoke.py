...
async def test_ST_A03_スラッシュコマンドがサーバーに登録されている(running_bot):
    ...
    assert {"health_check", "register", "auth", "update_grades", "edit_student", "list_students", "delete_student", "export_students", "import_students"} <= {command.name for command in commands}
