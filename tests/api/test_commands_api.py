...
def test_登録されるコマンドの一覧(command_payloads):
    assert [c["name"] for c in command_payloads] == ["health_check", "register", "auth", "update_grades", "edit_student", "list_students", "delete_student", "export_students", "import_students"]
...
def test_管理者用コマンドは説明でそれとわかる(command_payloads):
    for command in command_payloads:
        if command["name"] in {"register", "update_grades", "edit_student", "list_students", "delete_student", "export_students", "import_students"}:
