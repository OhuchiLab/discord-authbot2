...
5. 変更履歴を残すログ用チャンネル `authbot-logs` を作り、**管理者と Bot だけが見られる** ようにする (Bot に「チャンネルを見る」「メッセージを送信」を許可)。
   管理者の操作・認証完了・Bot の起動が「誰が・何を・どう変えたか」の形で投稿されます。
...
| 管理者 | CSV (ひな形: [docs/templates/import_students.csv](./docs/templates/import_students.csv)) を添付して `/import_students` を実行し、登録する人を確認して確定 | CSV の全員をまとめて登録 (1 行でも問題があれば何も登録せず、問題の行を表示) |
| 管理者 | `/list_students` を実行 (学年・認証の状態で絞り込み可) | 登録されている学生情報の一覧を、実行した管理者にだけ表示 |
| 管理者 | `/export_students` を実行 (CSV / msgpack) | 学生情報のファイルを、実行した管理者にだけ渡す |
| 管理者 | `/delete_student` で学生を指定し、内容を確認して削除 | 学生情報を削除し、認証済みの人は未認証の状態に戻す |
| 管理者 | `/edit_student` で学生を指定し、氏名・学籍番号・学年・メールアドレスの変更や Discord との紐付け解除を確認して確定 | 学生情報を変更し、認証済みならニックネーム・ロールにも反映 |
...
学生情報を保存するたび (と Bot の起動時) に、日時付きのコピーが `data/backups/` に自動で残ります (新しいものから 50 個)。
ディスクの故障に備えるなら、`.env` の `BACKUP_DIR` に外付けディスクや NAS の場所を指定してください。
復元の手順は [詳細設計書「database_backup.py」](./docs/detailed_design.md#database_backuppy--databasebackup-f14) を参照してください。
...
| 機能 | `tests/functional/` | 機能 F1〜F16 を、参加・DM・コマンドから通しで |
